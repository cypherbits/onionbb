...
class main_listener implements EventSubscriberInterface
{
    ...
    public function onionbb_load($event)
    {
        ...
        // Check IP whitelist
        if ($checkIP && !isset($checkIPList[$currentIP])) {
            http_response_code(403);
            die('Forbidden');
        }

        // Check user is connecting with Tor2Web
        if ($checkTor2Web) {
        ...
        // Check for hostname (block clients connecting only by server IP)
        if ($checkHostHeader) {
        ...
        if ($checkUserAgents) {
            if ($checkUserAgentsTB8) {
                ...
                if (!isset($useragentok[$currentUserAgent])) {
                    http_response_code(403);
                    die('Forbidden');
                }
            } else if (!str_starts_with($currentUserAgent, "Mozilla/5.0")) {
                http_response_code(403);
                die('Forbidden');
            }
        }
